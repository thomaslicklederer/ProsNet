within ProsNet.Utilities.Math.Functions;
...
algorithm
  ...
  E1 := if x < 1 then ProsNet.Utilities.Math.Functions.polynomial(x, a1)
     - log(x) else ProsNet.Utilities.Math.Functions.polynomial(x, a2)/(
    ProsNet.Utilities.Math.Functions.polynomial(x, b2)*x*exp(x));
