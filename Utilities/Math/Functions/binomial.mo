...
algorithm
  ...
  if k <= 0.5*n then
    binom := integer(ProsNet.Utilities.Math.Functions.fallingFactorial(
      n, k)/ProsNet.Utilities.Math.Functions.factorial(k));
  else
    binom := integer(ProsNet.Utilities.Math.Functions.fallingFactorial(
      n, n - k)/ProsNet.Utilities.Math.Functions.factorial(n - k));
  end if;
