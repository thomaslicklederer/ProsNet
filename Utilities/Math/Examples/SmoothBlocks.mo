...
model SmoothBlocks "Test model for smooth min and smooth max functions"
  ...
  IBPSA.Utilities.Diagnostics.AssertEquality assEquMax(threShold=0.08)
    annotation (Placement(transformation(extent={{40,20},{60,40}})));
  ProsNet.Utilities.Math.SmoothMin smoMin(deltaX=0.5)
    annotation (Placement(transformation(extent={{-20,-40},{0,-20}})));
  Modelica.Blocks.Math.Min Min annotation (Placement(transformation(extent={{
            -20,-80},{0,-60}})));
  IBPSA.Utilities.Diagnostics.AssertEquality assEquMin(threShold=0.08)
...
__Dymola_Commands(file="modelica://IBPSA/Resources/Scripts/Dymola/Utilities/Math/Examples/SmoothBlocks.mos"
        "Simulate and plot"),
