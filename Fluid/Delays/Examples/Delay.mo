...
// that is used in this model.
 package Medium = ProsNet.Media.Air(X_default={0.001, 0.999});
  ...
  ProsNet.Fluid.FixedResistances.PressureDrop res1(
    from_dp=true,
    m_flow_nominal=5,
    dp_nominal=5,
  ...
  ProsNet.Fluid.Sources.Boundary_pT sin(
    ...
    nPorts=1)             annotation (Placement(transformation(extent={{78,-4},
            {58,16}})));
  ProsNet.Fluid.FixedResistances.PressureDrop res2(
    from_dp=true,
    m_flow_nominal=5,
    dp_nominal=5,
    redeclare package Medium = Medium)
    annotation (Placement(transformation(extent={{26,-4},{46,16}})));
  ProsNet.Fluid.Delays.DelayFirstOrder del(
    m_flow_nominal=5,
    redeclare package Medium = Medium,
    nPorts=2,
...
__Dymola_Commands(file="modelica://ProsNet/Resources/Scripts/Dymola/Fluid/Delays/Examples/Delay.mos"
        "Simulate and plot"));
end Delay;
