...
partial model ValveParameters "Model with parameters for valves"
  ...
  parameter ProsNet.Fluid.Types.CvTypes CvData=ProsNet.Fluid.Types.CvTypes.OpPoint
    "Selection of flow coefficient"
   annotation(Dialog(group = "Flow Coefficient"));
  parameter Real Kv(
    fixed= if CvData==ProsNet.Fluid.Types.CvTypes.Kv then true else false)
    "Kv (metric) flow coefficient [m3/h/(bar)^(1/2)]"
  annotation(Dialog(group = "Flow Coefficient",
                    enable = (CvData==ProsNet.Fluid.Types.CvTypes.Kv)));
  parameter Real Cv(
    fixed= if CvData==ProsNet.Fluid.Types.CvTypes.Cv then true else false)
    "Cv (US) flow coefficient [USG/min/(psi)^(1/2)]"
  annotation(Dialog(group = "Flow Coefficient",
                    enable = (CvData==ProsNet.Fluid.Types.CvTypes.Cv)));
  parameter Modelica.Units.SI.Area Av(fixed=if CvData == ProsNet.Fluid.Types.CvTypes.Av
         then true else false) "Av (metric) flow coefficient" annotation (
      Dialog(group="Flow Coefficient", enable=(CvData == ProsNet.Fluid.Types.CvTypes.Av)));
  ...
  parameter Modelica.Units.SI.PressureDifference dpValve_nominal(
    displayUnit="Pa",
    min=0,
    fixed=if CvData == ProsNet.Fluid.Types.CvTypes.OpPoint then true else false)
    "Nominal pressure drop of fully open valve, used if CvData=ProsNet.Fluid.Types.CvTypes.OpPoint"
    annotation (Dialog(group="Nominal condition", enable=(CvData == ProsNet.Fluid.Types.CvTypes.OpPoint)));
...
protected
  ...
  annotation(Dialog(group = "Flow Coefficient",
                    enable = (CvData==ProsNet.Fluid.Types.CvTypes.OpPoint)));
initial equation
  if  CvData == ProsNet.Fluid.Types.CvTypes.OpPoint then
  ...
  elseif CvData == ProsNet.Fluid.Types.CvTypes.Cv then
    ...
    assert(CvData == ProsNet.Fluid.Types.CvTypes.Av, "Invalid value for CvData.
