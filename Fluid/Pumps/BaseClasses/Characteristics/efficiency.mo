within ProsNet.Fluid.Pumps.BaseClasses.Characteristics;
function efficiency "Flow vs. efficiency characteristics for fan or pump"
  extends Modelica.Icons.Function;
  input ProsNet.Fluid.Pumps.BaseClasses.Characteristics.efficiencyParameters per
...
algorithm
  if n == 1 then
    ...
    rat:=V_flow/
            ProsNet.Utilities.Math.Functions.smoothMax(
              x1=r_N,
              x2=0.1,
              deltaX=delta);
    i :=1;
    ...
    eta:=ProsNet.Utilities.Math.Functions.cubicHermiteLinearExtrapolation(
                x=rat,
                x1=per.V_flow[i],
                x2=per.V_flow[i + 1],
                y1=per.eta[i],
