...
model FlowMachineInterface
  ...
  import cha = ProsNet.Fluid.Pumps.BaseClasses.Characteristics;

  constant Boolean homotopyInitialization = true "= true, use homotopy method"
    annotation(HideResult=true);

  parameter ProsNet.Fluid.Pumps.Data.Generic per
    "Record with performance data"
    annotation (choicesAllMatching=true,
      Placement(transformation(extent={{60,-80},{80,-60}})));

  parameter ProsNet.Fluid.Pumps.BaseClasses.Types.PrescribedVariable preVar=ProsNet.Fluid.Pumps.BaseClasses.Types.PrescribedVariable.Speed
...
protected
  ...
  final parameter Boolean prePre=
    preVar ==ProsNet.Fluid.Pumps.BaseClasses.Types.PrescribedVariable.PressureDifference or
    preVar ==ProsNet.Fluid.Pumps.BaseClasses.Types.PrescribedVariable.FlowRate
  ...
  final parameter
    ProsNet.Fluid.Pumps.BaseClasses.Characteristics.flowParametersInternal pCur1(
    final n=nOri,
  ...
  parameter
    ProsNet.Fluid.Pumps.BaseClasses.Characteristics.flowParametersInternal pCur2(
    final n=nOri + 1,
    ...
    "Volume flow rate vs. total pressure rise with correction for pump resistance added";
  parameter
    ProsNet.Fluid.Pumps.BaseClasses.Characteristics.flowParametersInternal pCur3(
    final n=nOri + 2,
  ...
  parameter Real powDer[size(per.power.V_flow,1)]=
   if per.use_powerCharacteristic then
     ProsNet.Utilities.Math.Functions.splineDerivatives(
                   x=per.power.V_flow,
                   y=per.power.P,
                   ensureMonotonicity=ProsNet.Utilities.Math.Functions.isMonotonic(x=per.power.P,
                                                                                     strict=false))
   else
...
function getPerformanceDataAsString
  input ProsNet.Fluid.Pumps.BaseClasses.Characteristics.flowParameters pressure
      "Performance data";
...
initial equation
  ...
  assert(ProsNet.Utilities.Math.Functions.isMonotonic(x=per.pressure.V_flow, strict=true) and
...
the simulation stops.");
  ...
  if (haveVMax and haveDPMax) or (nOri == 2) then  // ----- Curve 1
    ...
    preDer1= ProsNet.Utilities.Math.Functions.splineDerivatives(x=pCur1.V_flow,
      y=pCur1.dp);
  ...
  elseif haveVMax or haveDPMax then  // ----- Curve 2
    ...
    preDer1= zeros(nOri);
    preDer2= ProsNet.Utilities.Math.Functions.splineDerivatives(x=pCur2.V_flow,
      y=pCur2.dp);
    preDer3= zeros(nOri + 2);
  else  // ----- Curve 3
    ...
    preDer1= zeros(nOri);
    preDer2= zeros(nOri + 1);
    preDer3= ProsNet.Utilities.Math.Functions.splineDerivatives(x=pCur3.V_flow,
      y=pCur3.dp);
  end if;
 ...
 motDer = if per.use_powerCharacteristic then zeros(size(per.motorEfficiency.V_flow,
    ...
    ensureMonotonicity=ProsNet.Utilities.Math.Functions.isMonotonic(x=per.motorEfficiency.eta,
      strict=false));
  hydDer = if per.use_powerCharacteristic then zeros(size(per.hydraulicEfficiency.V_flow,
    1)) elseif (size(per.hydraulicEfficiency.V_flow, 1) == 1) then {0}
     else ProsNet.Utilities.Math.Functions.splineDerivatives(x=per.hydraulicEfficiency.V_flow,
...
equation
  ...
  if (computePowerUsingSimilarityLaws == false) and preVar <> ProsNet.Fluid.Pumps.BaseClasses.Types.PrescribedVariable.Speed then
    r_N=1;
  else
  ...
  if per.use_powerCharacteristic then
    ...
    // a division by zero.
    eta = WFlo / ProsNet.Utilities.Math.Functions.smoothMax(x1=PEle, x2=1E-5, deltaX=1E-6);
    ...
    // a division by zero.
    PEle = WFlo / ProsNet.Utilities.Math.Functions.smoothMax(x1=eta, x2=1E-5, deltaX=1E-6);
    eta = etaHyd * etaMot;
