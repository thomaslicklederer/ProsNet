...
model PartialEffectivenessNTU
  ...
  extends ProsNet.Fluid.HeatExchangers.BaseClasses.PartialEffectiveness(
    sensibleOnly1=true,
    sensibleOnly2=true,
    ...
    mWat1_flow=0,
    mWat2_flow=0);
  import con = ProsNet.Fluid.Types.HeatExchangerConfiguration;
  import flo = ProsNet.Fluid.Types.HeatExchangerFlowRegime;
...
initial equation
  ...
  NTU_nominal = if (eps_nominal > 0 and eps_nominal < 1) then
    ProsNet.Fluid.HeatExchangers.BaseClasses.ntu_epsilonZ(
    eps=eps_nominal,
    Z=Z_nominal,
...
equation
  ...
  // Effectiveness
  eps = ProsNet.Fluid.HeatExchangers.BaseClasses.epsilon_C(
    UA=UA,
    C1_flow=C1_flow,
    C2_flow=C2_flow,
