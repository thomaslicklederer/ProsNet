...
function epsilon_ntuZ
  ...
  import f = ProsNet.Fluid.Types.HeatExchangerFlowRegime;
...
algorithm
  ...
  elseif (flowRegime == Integer(f.CounterFlow)) then// counter flow
   // a is constraining Z since eps is not defined for Z=1.
    a := smooth(1, if Z < 0.97 then Z else
      ProsNet.Utilities.Math.Functions.smoothMin(
      x1=Z,
      x2=0.98,
      deltaX=0.01));
  ...
  annotation(preferredView="info",
             inverse(NTU=ProsNet.Fluid.HeatExchangers.BaseClasses.ntu_epsilonZ(eps=eps, Z=Z, flowRegime=flowRegime)),
           smoothOrder=1,
