...
model TraceSubstanceConservation
  ...
  IBPSA.Fluid.Sources.Boundary_pT sin(redeclare package Medium = Medium,
      nPorts=1) "Air sink"
    annotation (Placement(transformation(extent={{100,-10},{80,10}})));
  IBPSA.Fluid.Sensors.MassFlowRate senMasFloIn(redeclare package Medium =
        Medium, allowFlowReversal=false) "Fluid mass flow rate at inlet"
    annotation (Placement(transformation(extent={{-40,10},{-20,-10}})));
  IBPSA.Fluid.Sensors.MassFlowRate senMasFloOut(redeclare package Medium =
        Medium, allowFlowReversal=false) "Fluid mass flow rate at outlet"
